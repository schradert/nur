{lib, withSystem, ...}: {
  flake.overlays.zellij-plugins = final: _: {
    zellijPlugins = withSystem final.stdenv.hostPlatform.system ({self', ...}: lib.genAttrs ["harpoon" "room" "jbz" "monocle" "multitask" "zellij-forgot"] (lib.flip lib.getAttr self'.packages));
  };
  perSystem = {
    pkgs,
    self',
