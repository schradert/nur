{
  perSystem = {lib, pkgs, ...}: {
    packages = lib.filterAttrs (_: lib.isDerivation) pkgs.zellijPlugins;
  };
  flake.overlays.zellij-plugins = final: _: {
    zellijPlugins = final.callPackage ({
      lib,
      stdenv,
      buildZellijPlugin,
      fetchFromGitHub,
      newScope,
    }: lib.makeScope newScope (zpkgs: {
      zide = stdenv.mkDerivation rec {
        pname = "zide";
        version = "3.1.0";
        src = fetchFromGitHub {
          owner = "josephschmitt";
