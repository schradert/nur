{inputs, ...}: {
  perSystem = {
    ...
    inherit (pkgs) androidenv lib fetchurl fetchFromGitHub mkShell runCommand stdenv gradle writeShellApplication mktemp;
    ...
    env = let
      ...
      '';
    in {
      JAVA_HOME = pkgs."jdk${versions.jdk}".home;
  ...
  in {
    ...
    packages.lockscreen-widgets = inputs.gradle2nix.builders.${system}.buildGradlePackage rec {
      ...
      passthru.generate-lock-file = writeShellApplication {
        ...
        runtimeEnv = env;
        runtimeInputs = [gradle mktemp inputs'.gradle2nix.packages.gradle2nix];
        text = ''
